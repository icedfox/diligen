...
app.get('/documents/:index', (req, res, next) => {

	let index = req.params.index -1;

	if (index < data.documents.length && index >= 0) {
		// the index requested is in the list
		res.status(200).send(data.documents[index]);
	} else {
		// the index requested is not in the list
